...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function escapeHTML(str) {
    // console.log("[AdminScript] escapeHTML: Chamada com:", str);
  ...
  function updateBotStatusUI(clientData) {
    ...
    if (clientData.status === "QR_CODE") {
      ...
      if (qrCodeValue && typeof qrCodeValue === "string" && qrCodeValue.trim() !== "") {
        ...
        console.warn("[AdminScript] updateBotStatusUI: QR Code string inválida. Payload:", clientData);
      ...
      console.log("[AdminScript] updateBotStatusUI: Status é READY. JID:", clientData.jid);
    ...
    } else if (["DISCONNECTED", "AUTH_FAILURE", "FATAL_ERROR", "RESTARTING"].includes(clientData.status)) {
      ...
      console.log(`[AdminScript] updateBotStatusUI: Status do bot: ${clientData.status}`);
      qrDisplay.innerHTML = svgIconSpinner;
      qrStatusText.textContent = escapeHTML(clientData.status);
      if (botActionsDiv) botActionsDiv.style.display = "none";
    } else {
      console.log("[AdminScript] updateBotStatusUI: Status do bot desconhecido. Payload:", clientData);
      qrDisplay.innerHTML = svgIconSpinner;
    ...
    updateSystemInfo({
        ...
        attendants: clientData.status === "READY" ? (Math.floor(Math.random() * 5) + 1) : 0,
        conversations: clientData.status === "READY" ? (Math.floor(Math.random() * 10) + 1) : 0,
    });
    console.log("[AdminScript] updateBotStatusUI: UI do status do bot atualizada.");
  }

  function updateSystemInfo(data) {
    // console.log("[AdminScript] updateSystemInfo: Chamada com data:", data);
    ...
    // console.log("[AdminScript] updateSystemInfo: Informações do sistema atualizadas na UI.");
  }
  ...
  function connectWebSocket() {
    ...
    websocket.onmessage = (event) => {
      ...
      console.log("[AdminScript] WebSocket.onmessage: Mensagem recebida (RAW):", rawData ? rawData.substring(0, 150) + "..." : "N/A");
      let data;
      try {
      ...
      if (data.clientId === sessionId || !data.clientId || ["bot_status_update", "status_update", "initial_status", "qr_code"].includes(data.type)) {
        console.log("[AdminScript] WebSocket.onmessage: Processando mensagem para o bot. Tipo:", data.type);
        if ((["status_update", "initial_status", "bot_status_update"].includes(data.type)) && data.payload) {
        ...
        console.log("[AdminScript] WebSocket.onmessage: Mensagem para clientId diferente ou tipo não relevante. Ignorando. Dados:", data);
      }
    };
