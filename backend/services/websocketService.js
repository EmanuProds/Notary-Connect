...
let whatsappServiceInstance // Instância do whatsappService
...
function initializeWebSocketServer(server, logFunction, bsInstance, fbInstance) {
  sendLog = logFunction
  whatsappServiceInstance = bsInstance
  ...
  wss.on("connection", (ws, req) => {
    ...
    if (parsedUrl.pathname === "/admin-qr") {
      ...
      if (agentId) {
        ...
        if (attendantClients.has(agentId)) {
          sendLog(
            `[WS] Tentativa de conexão duplicada para o atendente ${agentName} (${agentId}). Fechando conexão anterior.`,
            "warn",
          )
    ...
    ws.on("message", async (messageBuffer) => {
      ...
      try {
        ...
        sendLog(
          `[WS] << De ${ws.agentName || ws.agentId || ws.clientType || "desconhecido"}: ${messageString.substring(0, 200)}`,
          "debug",
        )
        ...
        switch (parsedMessage.type) {
          ...
          case "send_chat_message": // Enviado pelo chatActions.js (atendente)
            if (ws.clientType === ClientType.ATTENDANT_CHAT && parsedMessage.payload) {
              ...
              if (to && (text || media) && ws.agentId) {
                if (!whatsappServiceInstance || typeof whatsappServiceInstance.sendWhatsAppMessage !== "function") {
                  ...
                  ws.send(
                    JSON.stringify({
                      ...
                      success: false,
                      error: "Serviço Baileys indisponível.",
                      originalMessage: parsedMessage.payload,
                    }),
                  )
                  return
                }
                // Monta o conteúdo da mensagem para o Baileys
                const messageContentBaileys = {}
                ...
                if (media) {
                  sendLog(
                    `[WS] Envio de mídia solicitado para ${to} (ainda não implementado completamente): ${JSON.stringify(media).substring(0, 100)}`,
                    "info",
                  )
                ...
                const success = await whatsappServiceInstance.sendWhatsAppMessage(
                  to,
                ...
                ws.send(
                  JSON.stringify({
                    type: "message_sent_ack",
                    success,
                    originalMessageId: parsedMessage.payload.id,
                    sentMessageId: success ? "id_do_baileys" : null,
                  }),
                )
              } else {
                sendLog(
                  `[WS] send_chat_message inválido de ${ws.agentName}: 'to' e ('text' ou 'media') são obrigatórios.`,
                  "warn",
                )
                ws.send(
                  JSON.stringify({
          ...
          case "request_chat_list": // Atendente solicita lista de chats
            if (ws.clientType === ClientType.ATTENDANT_CHAT && ws.agentId) {
              if (
                !firebirdServiceInstance ||
                typeof firebirdServiceInstance.getConversationsForAttendant !== "function"
              ) {
                sendLog("[WS] Erro: firebirdServiceInstance.getConversationsForAttendant não disponível.", "error")
                ws.send(
                  JSON.stringify({ type: "chat_list_response", error: "Serviço de banco de dados indisponível." }),
                )
                return
              }
          ...
          case "request_chat_history": // Atendente solicita histórico de um chat
            if (ws.clientType === ClientType.ATTENDANT_CHAT && parsedMessage.conversationId) {
              ...
              const history = await firebirdServiceInstance.getConversationHistory(
                parsedMessage.conversationId,
                parsedMessage.limit,
                parsedMessage.offset,
              )
              ws.send(
                JSON.stringify({
                  type: "chat_history_response",
          ...
          case "take_chat": // Atendente assume um chat pendente
            if (ws.clientType === ClientType.ATTENDANT_CHAT && parsedMessage.conversationId && ws.agentId) {
              if (
                !firebirdServiceInstance ||
                typeof firebirdServiceInstance.assignConversationToAttendant !== "function"
              ) {
              ...
              const success = await firebirdServiceInstance.assignConversationToAttendant(
                parsedMessage.conversationId,
                ws.agentId,
                ws.agentName,
              )
              ...
              if (success) {
                broadcastToAttendants(
                  ...
                  ws.agentId,
                ) // Notifica outros atendentes, exceto o que assumiu
              }
            }
            break
          ...
          case "end_chat": // Atendente encerra um chat
            if (ws.clientType === ClientType.ATTENDANT_CHAT && parsedMessage.conversationId) {
              ...
              ws.send(
                JSON.stringify({ type: "end_chat_response", success, conversationId: parsedMessage.conversationId }),
              )
              if (success) {
                broadcastToAttendants({
                  type: "chat_closed_update",
                  payload: { conversationId: parsedMessage.conversationId, agentId: ws.agentId },
                })
              }
            }
            break
          ...
          default:
            sendLog(
              `[WS] Tipo de mensagem WS não tratada: ${parsedMessage.type} de ${ws.agentName || ws.agentId || ws.clientType}`,
              "warn",
            )
        }
      } catch (e) {
        sendLog(
          `[WS] Erro ao processar mensagem WS de ${ws.agentName || ws.agentId || ws.clientType}: ${e.message}. Dados: ${messageString.substring(0, 200)}`,
          "error",
        )
    ...
    ws.on("close", (code, reasonBuffer) => {
      ...
      sendLog(
        `[WS] Cliente WS desconectado (Tipo: ${ws.clientType}, Agente: ${ws.agentName || ws.agentId || "N/A"}). Código: ${code}, Razão: ${reason}`,
        "info",
      )
      if (ws.clientType === ClientType.ADMIN_QR) {
    ...
    ws.on("error", (error) => {
      sendLog(
        `[WS] Erro no WebSocket (Tipo: ${ws.clientType}, Agente: ${ws.agentName || ws.agentId || "N/A"}): ${error.message}`,
        "error",
      )
    })
  })
...
function sendMessageToAttendant(agentId, data) {
  ...
  if (attendantClient && attendantClient.readyState === WebSocket.OPEN) {
    ...
    attendantClient.send(message, (err) => {
      if (err)
        sendLog(
          `[WS CHAT] Erro ao enviar para atendente ${attendantClient.agentName || agentId}: ${err.message}`,
          "error",
        )
    })
    return true
...
function broadcastToAttendants(data, excludeAgentId = null) {
  ...
  attendantClients.forEach((client, agentId) => {
    if (agentId !== excludeAgentId && client.readyState === WebSocket.OPEN) {
      client.send(message, (err) => {
        if (err)
          sendLog(`[WS CHAT] Erro ao transmitir para atendente ${client.agentName || agentId}: ${err.message}`, "error")
      })
    }
  })
}
