      ...
      db.run("BEGIN TRANSACTION;", async (beginErr) => {
        ...
        try {
          ...
          db.run("COMMIT;", (commitErr) => {
            if (commitErr) {
              ...
              db.run("ROLLBACK;", (rollbackErr) => {
                if (rollbackErr)
                  globalSendLog(
                    `[SQLite] Erro crítico ao tentar rollback após falha no commit: ${rollbackErr.message}`,
                    "error",
                  )
                reject(commitErr)
              })
          ...
          db.run("ROLLBACK;", (rollbackErr) => {
            if (rollbackErr)
              globalSendLog(
                `[SQLite] Erro crítico ao tentar rollback após erro no callback: ${rollbackErr.message}`,
                "error",
              )
...
async function createTablesIfNotExists() {
  ...
  try {
    await run(createAttendantsTable)
    globalSendLog("[SQLite] Tabela ATTENDANTS verificada/criada.", "info")
    await run(createWhatsappSessionsTable)
    globalSendLog("[SQLite] Tabela WHATSAPP_SESSIONS verificada/criada.", "info")
    await run(createWhatsappAuthStoreTable)
    globalSendLog("[SQLite] Tabela WHATSAPP_AUTH_STORE verificada/criada.", "info")
    await run(createClientsTable)
    globalSendLog("[SQLite] Tabela CLIENTS verificada/criada.", "info")
    await run(createConversationsTable)
    globalSendLog("[SQLite] Tabela CONVERSATIONS verificada/criada.", "info")
    await run(createMessagesTable)
    globalSendLog("[SQLite] Tabela MESSAGES verificada/criada.", "info")
...
async function getAttendantByUsername(username) {
  /* ... (como antes) ... */
  const sql =
    "SELECT ID, USERNAME, PASSWORD_HASH, NAME, IS_ADMIN, SECTOR, DIRECT_CONTACT_NUMBER FROM ATTENDANTS WHERE UPPER(USERNAME) = UPPER(?)"
  try {
...
async function createAttendant(attendantData, dbInstance) {
  ...
  const sql =
    "INSERT INTO ATTENDANTS (USERNAME, PASSWORD_HASH, NAME, IS_ADMIN, SECTOR, DIRECT_CONTACT_NUMBER) VALUES (?, ?, ?, ?, ?, ?)"
  const params = [
  ...
  const defaultAttendants = [
    {
      USERNAME: "admin",
      NAME: "Administrador",
      PASSWORD_HASH: "$2b$10$lrxuqCVgRxuFs1NkkXs81ur0AIk6U6pjPfggt6gHdAbVF9PFf9vxu",
      IS_ADMIN: true,
      SECTOR: null,
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "joenis",
      NAME: "Joenis",
      PASSWORD_HASH: "$2b$10$Mw9FP48xSichgLiBbVJSsOS6UsNEr4qP9VUAO/6c7kvNyXRqyvxvu",
      IS_ADMIN: false,
      SECTOR: ["balcao_rcpn", "rcpj_rtd_protestos", "notas"],
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "luciene",
      NAME: "Luciene",
      PASSWORD_HASH: "$2b$10$lReSRjnSH19/3k/IkP281.WMERyvsIytlxl04TZ9Cg1j380eanNpq",
      IS_ADMIN: false,
      SECTOR: ["balcao_rcpn"],
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "manuelle",
      NAME: "Manuelle",
      PASSWORD_HASH: "$2b$10$zkgIY1.qleUhClDngUq17eV9TV.panx9MfBfkrKsnpe128ocogUP6",
      IS_ADMIN: false,
      SECTOR: ["rcpj_rtd_protestos"],
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "jessica",
      NAME: "Jéssica",
      PASSWORD_HASH: "$2b$10$dh2w8O8skoyekD.X/sFAgOX/7FGLEqrqbgTpN0vmDnP3IAFb0RPmS",
      IS_ADMIN: false,
      SECTOR: ["rgi"],
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "raiany",
      NAME: "Raiany",
      PASSWORD_HASH: "$2b$10$nstqua1mqKAdgXLXARfXceXMWT5gmKZwucdof4PZE1g/7fAIx9Mti",
      IS_ADMIN: false,
      SECTOR: ["notas"],
      DIRECT_CONTACT_NUMBER: null,
    },
    {
      USERNAME: "luciano",
      NAME: "Luciano",
      PASSWORD_HASH: "$2b$10$nOdtY32AYZTiPGZf74H7O.oyrePcb0nVI6HW/VhkhJpZFtrwHfB0S",
      IS_ADMIN: false,
      SECTOR: ["notas"],
      DIRECT_CONTACT_NUMBER: "5522981683531",
    },
  ]
    ...
    await executeTransaction(async (transactionDb) => {
      for (const attendant of defaultAttendants) {
        const existingAttendant = await new Promise((resolve, reject) => {
          transactionDb.get(
            "SELECT ID FROM ATTENDANTS WHERE UPPER(USERNAME) = UPPER(?)",
            [attendant.USERNAME],
            (err, row) => {
...
function serializeData(data) {
  ...
  const replacer = (key, value) => {
    if (
      ...
      (typeof value === "object" && value !== null && value.type === "Buffer" && Array.isArray(value.data))
    ) {
...
function deserializeData(jsonString) {
  ...
  const reviver = (key, value) => {
    if (
      typeof value === "object" &&
      value !== null &&
      value.__buffer__ !== undefined &&
      typeof value.__buffer__ === "string"
    ) {
  ...
  try {
    ...
    globalSendLog(
      `[SQLiteAuth] Erro ao desserializar JSON: ${e.message}. String: ${jsonString.substring(0, 100)}...`,
      "error",
    )
...
async function sqliteAuthStore(sessionId) {
  ...
  const clearAllData = async () => {
    ...
    try {
      ...
      // Atualizar o status da sessão para indicar limpeza
      const updateStatusSql = `
        ...
        LAST_UPDATED = datetime('now');
      `
      await run(updateStatusSql, [sessionId])
      globalSendLog(`[SQLiteAuth] Status da sessão ${sessionId} atualizado para CLEARED no DB.`, "info")

      return true
    } catch (error) {
      globalSendLog(
        `[SQLiteAuth] Erro ao limpar todos os dados de autenticação para session '${sessionId}': ${error.message}`,
        "error",
      )
      return false
    }
  ...
  if (!creds) {
    globalSendLog(
      `[SQLiteAuth] Nenhuma credencial encontrada para session '${sessionId}', inicializando com initAuthCreds().`,
      "info",
    )
    creds = initAuthCreds()
  } else {
...
async function updateWhatsappSessionStatus(sessionId, status, jid = null, lastQrCode = null) {
  ...
  const sql = `
    ...
    `
  try {
    await run(sql, [sessionId, status, jid, lastQrCode])
    globalSendLog(`[SQLite] Status da sessão Baileys '${sessionId}' atualizado para: ${status}`, "info")
  } catch (error) {
    globalSendLog(`[SQLite] Erro ao atualizar status da sessão Baileys '${sessionId}': ${error.message}`, "error")
  }
}
async function getWhatsappSession(sessionId) {
  ...
  try {
    return await get(sql, [sessionId])
  } catch (error) {
    globalSendLog(`[SQLite] Erro ao buscar sessão Baileys '${sessionId}': ${error.message}`, "error")
    return null
  }
}
...
async function findOrCreateConversation(clientJid, attendantUsername = null) {
  /* ... (como antes) ... */
  ...
  const conversation = await get(sql, params)
  if (conversation) {
  ...
  const insertSql =
    "INSERT INTO CONVERSATIONS (CLIENT_WHATSAPP_ID, STATUS, CREATED_AT, LAST_MESSAGE_TIMESTAMP, ATTENDANT_USERNAME) VALUES (?, 'PENDING', datetime('now'), datetime('now'), ?)"
  try {
    const result = await run(insertSql, [clientJid, attendantUsername])
    return await get(
      "SELECT c.*, att.NAME as ATTENDANT_NAME FROM CONVERSATIONS c LEFT JOIN ATTENDANTS att ON c.ATTENDANT_ID = att.ID WHERE c.ID = ?",
      [result.lastID],
    )
  } catch (error) {
...
async function saveMessage(messageData) {
  ...
  try {
    const result = await run(sql, [
      ...
      message_type,
      messageTimestamp,
      is_read_by_agent ? 1 : 0,
    ])
    globalSendLog(
      `[SQLite] Mensagem salva para conversa ${conversation_id}. ID da mensagem no DB: ${result.lastID}`,
      "info",
    )
...
async function getConversationHistory(conversationId, limit = 50, offset = 0) {
  ...
  try {
    ...
    return messages.map((msg) => ({
      ...msg,
      MESSAGE_CONTENT:
        msg.MESSAGE_TYPE !== "text" && msg.MESSAGE_CONTENT ? deserializeData(msg.MESSAGE_CONTENT) : msg.MESSAGE_CONTENT,
...
async function getConversationsForAttendant(agentUsername, tabType = "active", searchTerm = null) {
  /* ... (como antes) ... */
  globalSendLog(
    `[SQLite] Buscando conversas para atendente ${agentUsername}, aba: ${tabType}, busca: ${searchTerm || "Nenhuma"}`,
    "info",
  )
  let sql = `
...
async function assignConversationToAttendant(conversationId, agentUsername, agentName) {
  ...
  try {
    ...
    globalSendLog(
      `[SQLite] Nenhuma conversa encontrada/atualizada para atribuição. ID: ${conversationId}, Atendente: ${agentUsername}`,
      "warn",
    )
    return null
  } catch (error) {
    globalSendLog(
      `[SQLite] Erro ao atribuir conversa ${conversationId} para ${agentUsername}: ${error.message}`,
      "error",
    )
    return null
  }
...
async function closeConversation(conversationId, agentUsername) {
  ...
  try {
    ...
    globalSendLog(
      `[SQLite] Nenhuma conversa ativa encontrada para fechar. ID: ${conversationId}, Atendente: ${agentUsername}`,
      "warn",
    )
    return null
  } catch (error) {
...
async function markMessagesAsReadByAgent(conversationId, agentUsername) {
  /* ... (como antes) ... */
  globalSendLog(
    `[SQLite] Marcando mensagens como lidas para conversa ${conversationId} pelo atendente ${agentUsername}`,
    "info",
  )
  const attendant = await getAttendantByUsername(agentUsername)
  if (!attendant) {
  ...
  try {
    ...
    globalSendLog(
      `[SQLite] Erro ao marcar mensagens como lidas para conversa ${conversationId}: ${error.message}`,
      "error",
    )
    throw error
  }
